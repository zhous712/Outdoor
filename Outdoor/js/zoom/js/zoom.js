(function ($) {
    var arrPath = $("#hfImagePath").val().split("|");
    var newLine = "";
    var directoryObj = $("#hfDirectory").val();
    $.each(arrPath, function () {
        // newLine += "<div><img src=\"" + ('http://' + directoryObj + this) + "\" width=\"600px\" height=\"600px\" /></div>";
        if (this != '' && this != null) {
            var arrPathAndSort = this.split(",");
            if (arrPathAndSort.length > 1) {
                newLine += " <div style=\"float:left;border:1px solid grey;\"><a href=\"" + ('http://' + directoryObj + arrPathAndSort[0]) + "\"><img src=\"" + ('http://' + directoryObj + arrPathAndSort[0].substring(0, arrPathAndSort[0].lastIndexOf('.')) + 's' + arrPathAndSort[0].substring(arrPathAndSort[0].lastIndexOf('.'))) + "\" /></a>";
                newLine += "<div style=\"float:left;\"><img style=\"cursor:pointer\" src=\"../js/zoom/img/addimg.jpg\" onclick=\"addImg('" + arrPathAndSort[0] + "')\"/><br/><input url=\"" + arrPathAndSort[0] + "\" type=\"text\" style=\"width:30px;margin: 10px 0px 0px 5px\" value=\"" + arrPathAndSort[1] + "\"/></div></div>";
            }
            else if (arrPathAndSort.length == 1) {
                newLine += " <div style=\"float:left;\"><a href=\"" + ('http://' + directoryObj + arrPathAndSort[0]) + "\"><img src=\"" + ('http://' + directoryObj + arrPathAndSort[0].substring(0, arrPathAndSort[0].lastIndexOf('.')) + 's' + arrPathAndSort[0].substring(arrPathAndSort[0].lastIndexOf('.'))) + "\" /></a></div>";
            }
        }
    });
